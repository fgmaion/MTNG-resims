...
# Estimate the black-hole mass to stellar-mass relation
zoom_mbh_mstar = {}

outdir = os.path.join(paths.RESULTS_DIR, "mbh_mstar")
os.makedirs(outdir, exist_ok=True)
for name in name_list:
    zoom_mbh_mstar[name] = zoom_split[name].bh_mstar(sel_mask=zoom_sel[name], nbins=Nbins_bh_mstar)
    np.save(os.path.join(outdir, "mbh_mstar_{}_Nbins{:d}".format(name, Nbins_bh_mstar)), [zoom_mbh_mstar[name]])
