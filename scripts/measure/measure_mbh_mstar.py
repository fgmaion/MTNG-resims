...
for i in range(len(name_list)):
    zoom_mbh_mstar[name_list[i]] = zoom_split[name_list[i]].bh_mstar(sel_mask=zoom_sel[name_list[i]], nbins=Nbins_bh_mstar)
    np.save("/cosmos_storage/home/fgmaion/MTNG-resims/results/mbh_mstar/mbh_mstar_{}_Nbins{:d}".format(name_list[i], Nbins_bh_mstar), [zoom_mbh_mstar[name_list[i]]])
