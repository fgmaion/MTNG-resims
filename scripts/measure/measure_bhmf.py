...
zoom_bhmf = {}
outdir = os.path.join(paths.RESULTS_DIR, "bhmf")
os.makedirs(outdir, exist_ok=True)
for name in name_list:
    zoom_bhmf[name] = zoom_split[name].bh_mf(sel_mask=zoom_sel[name], nbins=Nbins_bhmf)
    np.save(os.path.join(outdir, "bhmf_{}_Nbins{:d}".format(name, Nbins_bhmf)), [zoom_bhmf[name]])
