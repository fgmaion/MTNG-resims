...
outdir = os.path.join(paths.RESULTS_DIR, "smhm_rel")
os.makedirs(outdir, exist_ok=True)
for name in name_list:
    zoom_smhm[name] = zoom_split[name].smhm_ratio(sel_mask=zoom_sel[name], nbins=Nbins_smhm)
    np.save(os.path.join(outdir, "smhm_relation_{}_Nbins{:d}".format(name, Nbins_smhm)), [zoom_smhm[name]])
