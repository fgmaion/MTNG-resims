...
for i in range(len(name_list)):
    zoom_smhm[name_list[i]] = zoom_split[name_list[i]].smhm_ratio(sel_mask=zoom_sel[name_list[i]], nbins=Nbins_smhm)
    np.save("/cosmos_storage/home/fgmaion/MTNG-resims/results/smhm_rel/smhm_relation_{}_Nbins{:d}".format(name_list[i], Nbins_smhm), [zoom_smhm[name_list[i]]])
