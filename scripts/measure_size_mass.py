...
for i in range(len(name_list)):
    zoom_rhalf[name_list[i]] = zoom_split[name_list[i]].rhalf_m2half(sel_mask=zoom_sel[name_list[i]], nbins=Nbins_rhalf)
    np.save("/cosmos_storage/home/fgmaion/MTNG-resims/results/size_mass_rel/rhalf_m2half_{}_Nbins{:d}".format(name_list[i], Nbins_rhalf), [zoom_rhalf[name_list[i]]])
