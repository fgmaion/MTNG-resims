...
for i in range(k):
    ...
    for j in range(len(test_sel)):
        test_x = torch.asarray(utils.pars(test_sel[j], np.log10(zoom_smf[name_list[test_sel[j]]]['mstar'][0])), dtype=torch.float)
        observed_pred = likelihood(model(torch.asarray(test_x, dtype=torch.float)), noise=torch.asarray(err_log_smf[-test_x.shape[0]:]**2, dtype=torch.float))        

        # Get upper and lower confidence bounds
        lower, upper = observed_pred.confidence_region()

        diff = np.log10(zoom_smf[name_list[test_sel[j]]]['smf'][0]) - observed_pred.mean.detach().numpy()

        ax.plot(np.log10(zoom_smf[name_list[test_sel[j]]]['mstar'][0]), diff, color='gray', lw=1, alpha=0.9)
...
ax.fill_between(mstar_array, -2*(0.1+err_log_smf), 2*(0.1+err_log_smf), alpha=0.2, color='C0', edgecolor=None, label='2$\sigma$')
ax.fill_between(mstar_array, -(0.1+err_log_smf), 0.1+err_log_smf, alpha=0.5, color='C0', edgecolor=None, label='1$\sigma$')

plt.savefig("/cosmos_storage/home/fgmaion/MTNG-resims/scripts/cross-validation/cv_k3_SMF_nbins10.pdf", bbox_inches='tight')