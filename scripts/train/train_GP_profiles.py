...
for prof_type in prof_types:
   for obj_type in obj_types:
      ...
      def pars(i, r):
          ...
          arr = np.vstack( ( r, np.ones(len(r)) * wind_en[i],\
                              np.ones(len(r)) * wind_vel[i],\
                              np.ones(len(r)) * rho_rec[i],\
                              np.ones(len(r)) * sf_ts[i],\
                              np.ones(len(r)) * ef_kin[i],\
                              np.ones(len(r)) * ef_high[i],\
                              np.ones(len(r)) * f_re[i])).T
      ...
      for i in range(len(name_list)):
          if obj_type == 'clusters':
              zoom_prof[name_list[i]] = np.load("/cosmos_storage/home/fgmaion/MTNG-resims/results/profiles/prof_clusters_{}.npy".format(name_list[i], prof_type), allow_pickle=True).item()
          elif obj_type == 'groups':
              zoom_prof[name_list[i]] = np.load("/cosmos_storage/home/fgmaion/MTNG-resims/results/profiles/prof_groups_{}.npy".format(name_list[i], prof_type), allow_pickle=True).item()
      ...
      #            'likelihood_state_dict':likelihood.state_dict()}, save_path + "model_rho")
      
      torch.save(model, save_path+"full_model_rho_{}_{}.pth".format(obj_type, prof_type))
      torch.save(likelihood, save_path+"full_likelihood_rho_{}_{}.pth".format(obj_type, prof_type))
