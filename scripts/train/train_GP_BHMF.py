...
for i in range(len(name_list)):
    zoom_bhmf[name_list[i]] = np.load(os.path.join(paths.RESULTS_DIR, "bhmf", "bhmf_{}_Nbins{:d}.npy".format(name_list[i], Nbins_bhmf)), allow_pickle=True)[0]

# Filter NaNs
for i in range(len(name_list)):
    mask = ~np.isnan(zoom_bhmf[name_list[i]]['bhmf']) & ~np.isnan(zoom_bhmf[name_list[i]]['mbh'])
    
    zoom_bhmf[name_list[i]]['bhmf'] = zoom_bhmf[name_list[i]]['bhmf'][mask]
    zoom_bhmf[name_list[i]]['mbh'] = zoom_bhmf[name_list[i]]['mbh'][mask]
...
torch.save(model, os.path.join(save_path, "full_model_bhmf.pth"))
torch.save(likelihood, os.path.join(save_path, "full_likelihood_bhmf.pth"))
