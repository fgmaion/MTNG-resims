...
zoom_mbh = {}
for i in range(len(name_list)):
    zoom_mbh[name_list[i]] = np.load("/cosmos_storage/home/fgmaion/MTNG-resims/results/mbh_mstar/mbh_mstar_{}_Nbins{:d}.npy".format(name_list[i], Nbins_mbh_mstar), allow_pickle=True)[0]
...
torch.save(model, save_path+"full_model_mbh_mstar.pth")
torch.save(likelihood, save_path+"full_likelihood_mbh_mstar.pth")
